...
pub fn print_command_help(command: &str) -> bool {
    if command == "stdio" {
        println!(
            "{}",
            r#"
agent-browser stdio - Machine-native NDJSON command mode

Usage: agent-browser stdio

Reads one JSON request per line from stdin and writes one JSON response per line to stdout.
Each request must include a `command` array using the same argument tokens you would pass
to the CLI directly.

Request shape:
  {"id":"req-1","command":["open","https://example.com"]}
  {"id":"req-2","session":"alt","command":["snapshot","-i"]}

Response shape:
  {"id":"req-1","session":"default","success":true,"action":"navigate","data":{...}}
  {"id":"req-2","session":"alt","success":false,"error":"Unknown command: chat","error_type":"unknown_command"}

Notes:
  - Requests are newline-delimited JSON, not a JSON array.
  - `session` is optional and defaults to the current CLI session/config.
  - Commands run through the same daemon-backed execution path as the normal CLI.
"#
            .trim()
        );
        return true;
    ...
    if !crate::commands::is_supported_command(command) {
        return false;
    }
...
pub fn print_help() {
    println!(
        r#"
agent-browser - stripped browser automation skeleton for agents
...
Supported Commands:
  ...
  stdio                      Machine-native NDJSON request/response mode
...
Options:
  ...
  --args <args>              Browser launch args (or AGENT_BROWSER_ARGS)
...
Examples:
  ...
  echo '{{"id":"1","command":["snapshot","-i"]}}' | agent-browser stdio
...
Command Chaining:
  ...
  agent-browser fill @e1 "user@example.com" && agent-browser click @e3
