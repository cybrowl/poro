...
import {
  TextAreaHistory as TextAreaHistory,
  type TextAreaHistoryOptions,
} from "./history";
...
export class InputEnhancer {
    ...
    if (e.detail == 2 && currentChar != "\n" && currentChar != " ") {
      ...
      requestAnimationFrame(() => {
        ...
        while (
          ...
          this.isWordCharacter(this.textarea.value[endPosition]) ==
            isWordChar &&
  ...
  private handleKeyDown(e: KeyboardEvent) {
    ...
    if (key === "enter") {
      ...
      key == "tab" &&
      !this.escapePressed &&
      !this.settings.disableTabCapture
    ) {
   ...
   */
  public toggleLinePrefix(
    prefix: string,
    whitespace: "attach" | "detach" = "attach"
  ) {
    const selection = this.getSelection();
  ...
  public getCursorXY(selectionPoint: number = this.textarea.selectionStart) {
    ...
    for (const prop of copyStyle) {
      ...
      div.style[prop as unknown as number] =
        copyStyle[prop as unknown as number];
    }
    const swap = ".";
    const inputValue =
