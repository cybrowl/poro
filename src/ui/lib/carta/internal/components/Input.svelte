...
<script lang="ts">
  ...
  export const resize = () => {
    ...
    if (
      elem &&
      (coords.top < 0 ||
        coords.top + carta.input.getRowHeight() >=
  ...
  function speculativeHighlight(value: string) {
    ...
    if (highlightElem) {
      try {
        const html = speculativeHighlightUpdate(
          currentlyHighlightedValue,
          value,
          currentOverlay
        );
  ...
  const highlightNestedLanguages = debounce(async (text: string) => {
    ...
    const hl = await import("$lib/carta/internal/highlight");
  ...
  let displayedOverlay = $derived(
    overlay && overlay.timestamp > speculativeOverlay.timestamp
      ? overlay
      : speculativeOverlay
  );
