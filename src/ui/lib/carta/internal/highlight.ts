...
export type Language = Intellisense<
  shiki.BundledLanguage | CustomMarkdownLangName
>;
...
export type ShikiOptions = {
  themes?: Array<
    shiki.ThemeInput | shiki.StringLiteralUnion<shiki.BundledTheme>
  >;
  langs?: (
    | shiki.LanguageInput
...
type CustomMarkdownLangName = Awaited<
  (typeof import("./assets/markdown"))["default"]["name"]
>;
type DefaultLightThemeName = Awaited<
  (typeof import("./assets/theme-light"))["default"]["name"]
>;
type DefaultDarkThemeName = Awaited<
  (typeof import("./assets/theme-dark"))["default"]["name"]
>;
 ...
 */
export const isThemeRegistration = (
  theme: Theme
): theme is shiki.ThemeRegistration => typeof theme == "object";
...
export type ShikiHighlighter = shiki.HighlighterGeneric<
...
export async function loadHighlighter(
  ...
  manager.shikiHighlighter.loadLanguage(
    ...(langsToLoad as shiki.LanguageInput[])
  );
  ...
  if (isSingleTheme(theme)) {
    ...
    if (isThemeRegistration(theme)) {
      ...
      themeRegistration = (
        await shiki.bundledThemes[theme as shiki.BundledTheme]()
      ).default;
    }

    themeHash = themeAndHighlightingHash(
      themeRegistration.name ?? "unknown",
      highlightingRules
    );
    ...
    if (!existingHighlighter) {
      const langClone: shiki.ThemeRegistration =
        structuredClone(langDefinition);
    ...
    if (isThemeRegistration(light)) {
      ...
      lightRegistration = (
        await shiki.bundledThemes[light as shiki.BundledTheme]()
      ).default;
    }
    ...
    if (isThemeRegistration(dark)) {
      ...
      darkRegistration = (
        await shiki.bundledThemes[dark as shiki.BundledTheme]()
      ).default;
    }
    ...
    const existingHighlighter = manager.highlighters.find(
      (highlighter) =>
        highlighter.settings.themeHash === lightHash ||
        highlighter.settings.themeHash === darkHash
    );

    if (!existingHighlighter) {
      const lightClone: shiki.ThemeRegistration =
        structuredClone(lightRegistration);
      const darkClone: shiki.ThemeRegistration =
        structuredClone(darkRegistration);
    ...
    codeToHtml: (code) => {
      if (isSingleTheme(theme)) {
        ...
        return manager.shikiHighlighter.codeToHtml(code, {
          lang: langHash,
          themes: {
            light: (themeHash as { light: string; dark: string })
              .light as string,
 ...
 */
export const loadNestedLanguages = async (
