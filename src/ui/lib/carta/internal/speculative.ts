 ...
 */
export function speculativeHighlightUpdate(
  from: string,
  to: string,
  currentHTML: string
): string {
  ...
  const onAddedCharacter = (char: string) => {
    ...
    span.textContent =
      text.slice(0, writingPosition.char) +
      char +
      text.slice(writingPosition.char);
  ...
  const onRemovedCharacter = (char: string) => {
    ...
    span.textContent =
      text.slice(0, readingPosition.char) +
      text.slice(readingPosition.char + 1);
    if (span.textContent === "") {
      span.remove();
    }
...
function isAtEndOfLine(position: Position, lines: Element[]): boolean {
  ...
  return (
    position.span >= line.children.length - 1 && isAtEndOfSpan(position, lines)
  );
}
